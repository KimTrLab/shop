...
public class BoardController {
    ...
    @GetMapping("/boards")
    public String getMethodName(PageVO pageVO, Model model) {
    ...
    public String postMethodName(
        ...
        // @ModelAttribute vo객체로 받는다.. 파라미티변수명과
        // vo의 변수명이 같으면 자동으로 매핑해 준다.
        // 글쓰기 화면에서 writer, title, content가 입력되면
        // 세 개의 정보를 boardvo변수로 받겠다.. 예전 DTO 개념
        System.out.println(boardvo.toString());
    ...
    // @ResponseBody
    // public ResponseEntity<Resource> displayFile(@PathVariable("id") Long id) {
    //     BoardFileVO fileVO = boardService.displayFile(id);
    //     String fullPath = fileVO.getFilePath()
    //             + File.separator
    //             + fileVO.getSavedName();
    ...
    @GetMapping("/download/{id}")
    @ResponseBody
    public ResponseEntity<Resource> downloadFile(@PathVariable("id") String fileName) {
